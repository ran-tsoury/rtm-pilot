...
function derivePersistedExperienceStatus(row) {
  if (
    !row ||
    typeof row !== "object"
  ) {
  ...
  const hasSelected =
    typeof row.selected_action === "string" &&
    row.selected_action.trim() !== "";

  const hasExecuted =
    typeof row.executed_action === "string" &&
  ...
  const hasConfidence =
    row.reliability_confidence !== null &&
    row.reliability_confidence !== undefined &&
    row.reliability_confidence !== "UNKNOWN";

  if (hasConfidence) {
    if (!hasOutcome || !hasExecuted || !hasSelected) {
      throw new Error(
        "Persisted intervention lifecycle is inconsistent"
      );
    }

    return EXPERIENCE_STATUS.CONFIDENCE;
  }

  if (hasOutcome) {
    if (!hasExecuted || !hasSelected) {
      throw new Error(
        "Persisted intervention lifecycle is inconsistent"
      );
    }

    return EXPERIENCE_STATUS.OUTCOME;
  }
  ...
  if (hasExecuted) {
    ...
    return EXPERIENCE_STATUS.EXECUTED;
  }

  if (hasSelected) {
    return EXPERIENCE_STATUS.SELECTED;
  }
  ...
  if (
    ...
    return EXPERIENCE_STATUS.SUGGESTED;
  }
...
export function createStatePersistence(
  ...
  async function loadJourneyState() {
    ...
    return Object.freeze({
      ...
      currentLoad:
        row.current_load ?? null,

      supportLevel:
        row.support_mode ?? null,
